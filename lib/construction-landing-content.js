export const constructionLandingFallback = {
...
export function mergeConstructionLandingContent(data) {
  if (!data) return constructionLandingFallback;

  const merged = {...constructionLandingFallback, ...data};

  for (const key of nestedKeys) {
    merged[key] = {...constructionLandingFallback[key], ...data[key]};
  }

  merged.services = data.services?.length
    ? data.services
    : constructionLandingFallback.services;
  merged.process.steps = data.process?.steps?.length
    ? data.process.steps
    : constructionLandingFallback.process.steps;
  merged.audience.items = data.audience?.items?.length
    ? data.audience.items
    : constructionLandingFallback.audience.items;
  merged.testimonials = data.testimonials?.length
    ? data.testimonials
    : data.testimonial?.quote
      ? [data.testimonial]
      : constructionLandingFallback.testimonials;
