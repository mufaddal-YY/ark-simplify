...
export default function ConstructionOverview({ data }) {
  const overview = data ?? defaultOverview;
  const constructionServices = overview.services?.length
    ? overview.services
    : defaultOverview.services;
  const scopeParagraphs = overview.scopeParagraphs?.length
    ? overview.scopeParagraphs
    : defaultOverview.scopeParagraphs;
            ...
            <div className="space-y-5 rounded-lg border border-brand-secondary/10 bg-white p-5 shadow-[0_22px_60px_rgba(54,59,79,0.06)] sm:p-6 lg:p-7">
              <div className="space-y-3">
                <p className="text-xs font-semibold tracking-[0.18em] text-brand-secondary/48 uppercase">
                  {overview.scopeEyebrow ?? defaultOverview.scopeEyebrow}
                </p>
              ...
              {scopeParagraphs.map((paragraph) => (
                <p
                  key={paragraph}
                  className="text-lg leading-9 text-brand-secondary/72 sm:text-xl"
                >
                  {paragraph}
                </p>
              ))}
